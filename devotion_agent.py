...
from esv_client import get_passage_text, ATTRIBUTION as ESV_ATTRIBUTION
...
def generate_devotion(passage: str) -> str:
    ...
    scripture_text = get_passage_text(passage)

    if scripture_text:
        # We supply the real ESV text ourselves — keep the model from reproducing
        # it (that is what trips Gemini's copyright/recitation filter).
        sections = f"**{angle_name}**\n**Application**\n**Prayer**"
        scripture_note = (
            "The Scripture passage is shown to the reader separately, above your "
            "output — do NOT reproduce the passage or include a Scripture section. "
            f"Begin directly with the **{angle_name}** section."
        )
    else:
        # No ESV text available (no API key / fetch failed) — prior behavior.
        sections = f"**Scripture**\n**{angle_name}**\n**Application**\n**Prayer**"
        scripture_note = (
            "For the Scripture section: include only the 1–2 verses most directly "
            f"relevant to today's angle ({angle_name}), in ESV. If the full passage "
            'is the focus, include only the passage reference (e.g. "Mark 15:1-15") '
            "— do not reproduce the full text.\n"
            "Begin directly with the Scripture section."
        )
...
Do not add introductory preamble.
"""

    body = generate(DEVOTION_SYSTEM_PROMPT, prompt, inject_kb_context=True).strip()

    if scripture_text:
        scripture_block = f"**Scripture**\n{passage} (ESV)\n\n{scripture_text}"
        return f"{scripture_block}\n\n{body}"
    return body
...
def _assemble_devotion(passage: str, body: str) -> str:
    """Prepend the Scripture block (real ESV text if available) to a body produced
    either from the pre-generated batch or from Gemini fallback. Body must NOT
    already contain a Scripture section."""
    esv = get_passage_text(passage)
    if esv:
        return f"**Scripture**\n{passage} (ESV)\n\n{esv}\n\n{body.strip()}"
    return f"**Scripture**\n{passage} (ESV)\n\n{body.strip()}"
...
def _load_pregenerated_devotion(passage: str) -> str | None:
    ...
    if not body:
        return None
    print(f"[devotion] Using pre-generated batch devotion for {passage}.")
    return _assemble_devotion(passage, body)
...
def run(force: bool = False):
    ...
    # Prefer the Logos batch (pre-generated by Claude via /logos-week); fall back
    # to live Gemini generation if no batch exists for this week.
    devotion = _load_pregenerated_devotion(passage)

    if devotion is None:
        print(f"[devotion] No batch found — generating {angle_name} devotion via Gemini for: {passage}")
        try:
